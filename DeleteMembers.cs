...
namespace TipnessAndroid
{
    ...
    public class DeleteMembers : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.listdeletemembers);
            btnBack = FindViewById<ImageView>(Resource.Id.imageBackButtonDeleteMemberList);
            ...
            dListField = FindViewById<EditText>(Resource.Id.field);

            dListSpinner = FindViewById<Spinner>(Resource.Id.category);

            btnDListSearch = FindViewById<Button>(Resource.Id.searchbutton);
        ...
        private void Memberlistview_ItemLongClick(object sender, AdapterView.ItemLongClickEventArgs e)
        {
            ...
            builder.SetMessage("Are you sure you want to deactivate this member?");
        ...
        private void BtnDListSearch_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        con.Open();
                        MySqlCommand cmd = new MySqlCommand("SELECT `RId`, `fName`, `cNumber`, `gAccessName`, `sDate`, `eDate`, `aTraining`, `tName`, `bName` FROM `tblmembers` WHERE `isActive` = 1 AND " + cat + " like '%" + val + "%'", con);
